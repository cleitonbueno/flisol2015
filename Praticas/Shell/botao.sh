...
/bin/echo -e "\nGPIO[${PIN}]       -> OK"
/bin/echo "in" > /sys/class/gpio/gpio${PIN}/direction
/bin/echo -e "GPIO[${PIN}] INPUT -> OK\n"
...
while true;
do
	if [ "$(/bin/cat /sys/class/gpio/gpio${PIN}/value)" = "0" ]; then
		/bin/echo -e "BOTAO GPIO[${PIN}] -> ACIONADO"
	fi
	sleep $INTERVAL
done
